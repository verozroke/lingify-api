...
export class AuthService {
  ...
  async signin(dto: AuthSinginDto, req: Request, res: Response) {
    ...
    const foundUser = await this.prisma.user.findUnique({ where: { nickname } })
    ...
    const token = await this.signToken({ id: foundUser.id, nickname: foundUser.nickname })
  ...
  async signToken(args: { id: string, nickname: string }) {
    const payload = args
